...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (!guild)
        {
            ...
            handler.PSendSysMessage("Você nao e membro de uma guild.");
            return true;
        }

        AddGossipItemFor(player, 10, "Informacao da Guild", GOSSIP_SENDER_MAIN, 1);
        AddGossipItemFor(player, 10, "Criterio", GOSSIP_SENDER_MAIN, 2);
        AddGossipItemFor(player, 10, "Sair", GOSSIP_SENDER_MAIN, 100);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action) override
    {
        ...
        {
            case 1:
                AddGossipItemFor(player, 10, acore::StringFormat("> Numero de Participantes %u", player->GetGuild()->GetMemberCount()), GOSSIP_SENDER_MAIN, 1);
                AddGossipItemFor(player, 10, ">> Ir para menu principal", GOSSIP_SENDER_MAIN, 99);
                SendGossipMenuFor(player, 1, creature->GetGUID());
                break;
            case 2: // Вклад в ги
