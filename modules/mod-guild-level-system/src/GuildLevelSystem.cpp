/*
* This file is part of the acoreCore Project. See AUTHORS file for Copyright information
*
...
GuildCriteriaProgressStruct* GuildCriteria::GetProgress(uint32 criteriaID, bool forceCreate /*= false*/)
{
    ...
    if (forceCreate)
    {
        ...
        LOG_FATAL("modules.gls", "> GLS: Not found criteria progress (%u) after insert empty", criteriaID);
    }
...
GuildCriteriaStruct* GuildCriteria::GetCriteria(uint32 criteriaID)
{
    ...
    if (criteria == _guildCriteria.end())
        LOG_FATAL("modules.gls", "> GLS: Not found criteria (%u) for guild id (%u)", criteriaID, _guildID);
...
void GuildCriteria::RescalingCriterias()
{
    ...
    if (!members)
        LOG_FATAL("modules.gls", "> GLS: No members count in guild (%u)", _guildID);
...
void GuildCriteria::SaveToDB(uint32 criteriaID)
{
    ...
    for (uint32 i = 0; i < GLS_ITEMS_COUNT; ++i)
        itemsCount += Utils::ToString(progressStruct->ItemCount[i]) + ",";
...
std::string const GuildLevelSystem::GetItemLink(uint32 itemID, int8 index_loc)
{
    ...
    return acore::StringFormat("|%s|Hitem:%u:0:0:0:0:0:0:0:0|h[%s]|h|r", color.c_str(), itemID, name.c_str());
}
...
std::string const GuildLevelSystem::GetSpellLink(uint32 spellID, int8 index_loc)
{
    ...
    return acore::StringFormat("|cffffffff|Hspell:%u|h[%s]|h|r", spell->Id, spell->SpellName[index_loc]);
}
...
void GuildLevelSystem::LoadBaseCriteria()
{
    ...
    do
    {
        ...
        auto const& listItemIDTokens = acore::Tokenize(listItemID, ',', true);
        auto const& listItemCountTokens = acore::Tokenize(listItemCount, ',', true);
        ...
        {
            uint32 itemID = acore::StringTo<uint32>(listItemIDTokens[i]).value_or(0);
        ...
        for (uint32 i = 0; i < static_cast<uint32>(listItemCountTokens.size()); ++i)
            _data.ItemCount[i] = acore::StringTo<uint32>(listItemCountTokens[i]).value_or(0);
        ...
        auto const& listRewardSpellsTokens = acore::Tokenize(listRewardSpells, ',', true);
        ...
        {
            uint32 spellID = acore::StringTo<uint32>(listRewardSpellsTokens[i]).value_or(0);
...
void GuildLevelSystem::LoadStageProgress()
{
    ...
    if (!result)
    {
        LOG_WARN("modules.gls", "> Загружено 0 GLS стадий прогресса. Таблица `gls_stages_progress` пустая.");
        return;
    }
    ...
    do
    {
        ...
        if (!criteriaProgress)
        {
            LOG_ERROR("modules.gls", "> GLS: guild id (%u) is not exist in criteria progress!. Skip", guildID);
            continue;
        }
    ...
    LOG_INFO("modules.gls", ">> Загружено %u GLS стадий прогресса за %u мс", count, GetMSTimeDiffToNow(oldMSTime));
}
...
GuildCriteria* GuildLevelSystem::GetCriteriaProgress(uint32 guildid, bool forceCreate /*= false*/)
{
    ...
    if (forceCreate)
    {
        ...
        LOG_FATAL("modules.gls", "> GLS: Invalid create empty guild criteria for guild (%u)", guildid);
    }
...
void GuildLevelSystem::ShowAllCriteriaInfo(Player* player, Creature* creature)
{
    ...
    AddGossipItemFor(player, 10, acore::StringFormat("# Текущая стадия: %u", stageID), GOSSIP_SENDER_MAIN, 2);
    ...
    if (GetCountCriteriaInStage(stageID) != countDone)
    {
        ...
        for (auto const& itr : _guildCriteriaBase)
        {
            ...
            AddGossipItemFor(player, 10, acore::StringFormat("--- Критерий #%u. Прогресс (%u/%u)", criteriaID, countNow, countMax), GLS_GOSSIP_SHOW_CRITERIA_SENDER, gossipAction);
                ...
                for (uint32 i = 0; i < GLS_ITEMS_COUNT; ++i)
                {
                    ...
                    if (!itemLink.empty())
                        AddGossipItemFor(player, 10, acore::StringFormat("- %u. %s - %u/%u", i + 1, itemLink.c_str(), currentItems, maxItems), i, gossipAction);
                }
        }
    }
    else
    {
        ...
        if (player->GetRank() == 0 && GetNextStage(stageID) != stageID)
            AddGossipItemFor(player, 10, acore::StringFormat("# >> Завершить стадию - %u", countDone), GOSSIP_SENDER_MAIN, 4);
    }
...
void GuildLevelSystem::ShowCriteriaInfo(Player* player, Creature* creature, uint32 sender, uint32 action)
{
    ...
    AddGossipItemFor(player, 10, acore::StringFormat("# Детальная информация о критерии %u", criteriaID), sender, action);
    AddGossipItemFor(player, 10, "#", sender, action);
    ...
    for (uint32 i = 0; i < GLS_ITEMS_COUNT; ++i)
    {
        ...
        if (!itemLink.empty())
            AddGossipItemFor(player, 10, acore::StringFormat("- %u. %s - %u/%u", i + 1, itemLink.c_str(), currentItems, maxItems), i, action);
    }
    ...
    for (uint32 i = 0; i < GLS_SPELLS_REWARD_COUNT; ++i)
    {
        ...
        AddGossipItemFor(player, 10, acore::StringFormat("# %u. %s", i + 1, GetSpelllocale(spellID).c_str()), sender, action);
    }
...
void GuildLevelSystem::ShowRewardInfo(Player* player, Creature* creature, uint32 sender, uint32 action)
{
    ...
    AddGossipItemFor(player, 10, "#", sender, action);
    AddGossipItemFor(player, 10, acore::StringFormat("# Получение награды за критерий %u", criteriaID), sender, action);
    AddGossipItemFor(player, 10, "#", sender, action);
    ...
    for (uint32 i = 0; i < GLS_SPELLS_REWARD_COUNT; ++i)
    {
        ...
        AddGossipItemFor(player, 10, acore::StringFormat("# %u. %s%s", i + 1, stringSelect(selectShoose, i).c_str(), spellLink.c_str()), GLS_GOSSIP_GET_REWARDS_SENDER + i, action,
                         acore::StringFormat("Вы уверены, что хотите выбрать %s?", spellLink.c_str()), 0, false);
    }
...
void GuildLevelSystem::GetRewardsCriteria(Player* player, Creature* creature, uint32 sender, uint32 action)
{
    ...
    {
        LOG_FATAL("modules.gls", "> GLS: spellType(%u) >= GLS_SPELLS_REWARD_COUNT(%u)", spellType, GLS_SPELLS_REWARD_COUNT);
        return;
    }
...
void GuildLevelSystem::ShowInvestedMenu(Player* player, Creature* creature, uint32 sender, uint32 action)
{
    ...
    AddGossipItemFor(player, 10, "#", sender, action);
    AddGossipItemFor(player, 10, acore::StringFormat("# %s", itemLink.c_str()), sender, action);
    AddGossipItemFor(player, 10, acore::StringFormat("# Вложений - %u/%u", currentItems, maxItems), sender, action);
    AddGossipItemFor(player, 10, acore::StringFormat("# Осталось вложить - %u", itemsNeed), sender, action);
    AddGossipItemFor(player, 10, acore::StringFormat("# Прогресс выполнения - %0.2f%%", persents), sender, action);
    AddGossipItemFor(player, 10, "#", sender, action);
    AddGossipItemFor(player, 10, acore::StringFormat("- В инвентаре - %u", playerItemCount), sender, action);

    if (itemsNeed)
    {
        AddGossipItemFor(player, 10, "- Вложить выборочно", sender, action, "Введите количество", 0, true);
        AddGossipItemFor(player, 10, acore::StringFormat("- Вложить %s (%u)", itemCount == itemsNeed ? "до конца" : "всё что есть", itemCount), sender, criteriaID + GLS_GOSSIP_CRITERIA_ID_FULL,
                         acore::StringFormat("Вы уверены, что хотите вложить %s %u?", itemLink.c_str(), playerItemCount), 0, false);
    }
...
uint32 GuildLevelSystem::GetCountCriteriaInStage(uint32 stageID)
{
    ...
    LOG_FATAL("modules.gls", "> GLS: Not find stage %u", stageID);
...
uint32 GuildLevelSystem::GetNextStage(uint32 currStage)
{
    ...
    LOG_FATAL("modules.gls", "> GLS: Not found next stage after %u", currStage);
