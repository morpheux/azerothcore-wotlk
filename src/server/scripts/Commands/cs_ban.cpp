...
public:
    ...
    {
        QueryResult result = LoginDatabase.PQuery("SELECT FROM_UNIXTIME(bandate, '%%d-%%m-%%Y..%%H:%%I:%%s') as bandate, unbandate-bandate, active, unbandate, banreason, bannedby FROM account_banned WHERE id = '%u' ORDER BY bandate ASC", accountId);
        if (!result)
        {
